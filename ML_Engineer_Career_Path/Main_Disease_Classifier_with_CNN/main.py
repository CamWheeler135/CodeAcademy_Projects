...
def main():
    ## Data Cleaning. 
    # dc_dh = DataHandler(collection_path=Path('data/raw_files/'), 
    #                     save_path=Path('data/cleaned_files/'))
    ...
    usp_dh = DataHandler(collection_path=Path('data/cleaned_files'), 
                         save_path=Path('data/processed_files/kmers/kmers.csv'))
    usp = KmerPreprocessor(usp_dh, kmer_size=3 )
    usp.complete_preprocess()

    ## PCA Analysis
    pca_dh = DataHandler(collection_path=Path('data/processed_files/kmers/kmers.csv'),
